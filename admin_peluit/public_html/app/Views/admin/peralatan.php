...
<div class="main-content">
    <section class="section">
        <div class="section-header">
            <h1>Halaman Peralatan Zoom</h1>
            <div class="section-header-breadcrumb">
                <div class="breadcrumb-item active"><a href="<?= base_url() ?>/admin/dashboard">Dashboard</a></div>
                <div class="breadcrumb-item"><a href="<?= base_url() ?>/admin/peralatan">Data Master</a>
                </div>
                <div class="breadcrumb-item">Peralatan Zoom</div>
            </div>
        </div>
...
function ubah($id_alat, $nama_alat, $nomor_seri, $merk, $status) {
    remove_invalid();
    document.getElementById('nama').value = $nama_alat;
    document.getElementById('username').value = $merk;
    document.getElementById('nav').value = $nomor_seri;
    var baris = "";
    baris += '<div class="d-flex justify-content-between mb-4">';
    baris += '<button onclick="toogle_btn(0)" class="btn btn-danger">Batal</button>';
    baris += '<button id="btn_ubah" onclick="ubah_user(' + $id_alat + ')" class="btn btn-warning">Ubah</button>';
    baris +=
...
function tambah_user() {
    ...
    if (document.getElementById('nama').value == "" || isEmptyOrSpaces(document.getElementById('nama').value)) {
        ...
        formData.append('nama_alat', document.getElementById("nama").value);
        formData.append('merk', document.getElementById("username").value);
        formData.append('nomor_seri', document.getElementById("nav").value);

        $.ajax({
            url: "<?= base_url() ?>/admin/peralatan/set_peralatan",
            type: "POST",
            data: formData,
            contentType: false,
            processData: false,
...
function ubah_user($id) {
    ...
    if (document.getElementById('nama').value == "" || isEmptyOrSpaces(document.getElementById('nama').value)) {
        ...
        formData.append('merk', document.getElementById("username").value);
        formData.append('nomor_seri', document.getElementById("nav").value);
        formData.append('id_alat', $id);

        $.ajax({
            url: "<?= base_url() ?>/admin/peralatan/update_peralatan",
            type: "POST",
            data: formData,
            contentType: false,
            processData: false,
...
function get_data() {
    var table = $('#example').DataTable({
        ...
        "ajax": {
            "type": "GET",
            "url": "<?= base_url() ?>/admin/peralatan/get_peralatan",
            "dataSrc": "",
            "data": function(data) {
        ...
        "columns": [{
            ...
            },
            {
                ...
                "render": function(data, type, row) {
                    ...
                    button += '<a class="text-muted"><small>Nomor Seri</small></a>';

                    button += '<div class="bullet text-primary"></div>';
                    button += '<a class="text-primary"><small>' + row.nomor_seri + '</small></a>';
                    button += '</div>';
            ...
            },
            {
                ...
                "render": function(data, type, row) {
                    ...
                    button +=
                        ...
                        ",'" + row.nama_alat + "'" + ",'" + row.nomor_seri + "'" + ",'" + row.merk +
                        "'" +
                        ",'" + row.active + "'" +
...
function update_active($status, $id) {
    Swal.fire({
        ...
        text: "Aktifasi peralatan akan diubah.",
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            ...
            $.ajax({
                url: "<?= base_url() ?>/admin/peralatan/update_status",
                type: "POST",
                data: formData,
                contentType: false,
                processData: false,
...
function del($id) {
    Swal.fire({
        title: 'Apakah Anda Yakin ?',
        text: "Data Alat akan dihapus dan tidak dapat dipulihkan kembali.",
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        cancelButtonColor: '#d33',
        ...
        if (result.isConfirmed) {
            ...
            $.ajax({
                url: "<?= base_url() ?>/admin/peralatan/del_peralatan",
                type: "POST",
                data: formData,
                contentType: false,
                processData: false,
