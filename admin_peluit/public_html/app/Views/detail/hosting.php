                                        ...
                                        <div class="tab-pane fade" id="profile4" role="tabpanel"
                                            ...
                                            <div class="form-group">
                                                ...
                                                <textarea rows="4" class="form-control"
                                                    disabled="true"><?= $deskripsi ?></textarea>
                                            </div>
                                            <div class="form-group">
                                                <label>Spesifikasi Aplikasi</label>
                                                <textarea rows="4" class="form-control"
                                                    disabled="true"><?= $spesifikasi ?></textarea>
                                            </div>
                                            <small class="text-secondary">Akses SSH dan Database</small>
                                            <hr>
                                            <div class="form-row">
                                                <div class="form-group col-md-6">
                                                    <label>SSH</label>
                                                    <input id="ssh" type="text" class="form-control"
                                                        value="<?= $server_access ?>">
                                                </div>
                                                <div class="form-group col-md-6">
                                                    <label>Akses Database</label>
                                                    <input id="db" type="text" class="form-control"
                                                        value="<?= $db_access ?>">
                                                </div>
                                            </div>
                                            <div class="d-flex justify-content-end" id="div_simpan">
...
function access_button() {
    ...
    document.getElementById("div_simpan").innerHTML = simpan;
}

function update_status($status) {
    if ($status == 1) {
        if (document.getElementById("ssh").value == "") {
            Swal.fire({
                icon: 'error',
                title: 'Gagal...',
                text: 'Form SSH Kosong !'
            });
        } else if (isEmptyOrSpaces(document.getElementById('ssh').value)) {
            Swal.fire({
                icon: 'error',
                title: 'Gagal...',
                text: 'Form SSH Kosong !'
            });
        } else if (document.getElementById('ssh').value.toLowerCase().includes("'") || document.getElementById('ssh')
            .value
            .toLowerCase().includes('"') || document.getElementById('ssh').value.toLowerCase().includes("select") ||
            document.getElementById('ssh').value.toLowerCase().includes("delete") || document.getElementById('ssh')
            .value
            .toLowerCase().includes("drop")) {
            Swal.fire(
                'Gagal',
                "SSH tidak sesuai format.",
                'error'
            );
        } else if (!isEmptyOrSpaces(document.getElementById('db').value) && (document.getElementById('db').value
                .toLowerCase().includes("'") || document.getElementById('db').value
                .toLowerCase().includes('"') || document.getElementById('db').value.toLowerCase().includes("select") ||
                document.getElementById('db').value.toLowerCase().includes("delete") || document.getElementById('db')
                .value
                .toLowerCase().includes("drop"))) {
            Swal.fire({
                icon: 'error',
                title: 'Gagal...',
                text: 'Form Akses Database Salah !'
            });
        } else {
            Swal.fire({
...
function update_meeting() {
    if (document.getElementById("ssh").value == "") {
    ...
    } else if (document.getElementById('ssh').value.toLowerCase().includes("'") || document.getElementById('ssh').value
        .toLowerCase().includes('"') || document.getElementById('ssh').value.toLowerCase().includes("select") ||
        document.getElementById('ssh').value.toLowerCase().includes("delete") || document.getElementById('ssh').value
        .toLowerCase().includes("drop")) {
        Swal.fire(
            'Gagal',
            "SSH tidak sesuai format.",
        ...
        formData.append('ssh', document.getElementById("ssh").value);
        formData.append('db', document.getElementById("db").value);

        $.ajax({
            url: "<?= base_url() ?>/detail/update_hosting",
            type: "POST",
            data: formData,
            contentType: false,
            processData: false,
...
function access_status($status) {
    document.getElementById("div_simpan").innerHTML = "";
