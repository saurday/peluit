                    ...
                    <div class="card flex-grow-1 ">
                        ...
                        <div class="card-body">
                            <div class="form-group row align-items-center">
                                <label class="col-md-12 text-md-left text-left">Tanggal Kejadian</label>
                                <div class="col-lg-12 col-md-12">
                                    <input type="date" id="tgl" class="form-control">
                                </div>
                            </div>
                            <div class="form-group row align-items-center">
                                <label class="col-md-12 text-md-left text-left">Keluhan</label>
                                <div class="col-lg-12 col-md-12">
                                    <textarea rows="4" id="keluhan" class="form-control"></textarea>
                                </div>
                            </div>
                            ...
                            <hr>
                            <div class="form-group row align-items-center">
                                <label class="col-md-12 text-md-left text-left">Foto Dokumentasi Pelaporan</label>
                                <div class="col-lg-12 col-md-12">
...
function remove_invalid() {
    ...
    document.getElementById("keluhan").classList.remove("is-invalid");
...
function add_tiket() {
    ...
    if (document.getElementById('tgl').value == "") {
        ...
        Swal.fire({
            icon: 'error',
            title: 'Gagal...',
            text: 'Form Tanggal Kejadian Kosong !'
        });
    } else if (document.getElementById('keluhan').value == "") {
        document.getElementById('keluhan').classList.add("is-invalid");
        document.getElementById("keluhan").focus();
        Swal.fire({
            icon: 'error',
            title: 'Gagal...',
            text: 'Form Deskripsi Keluhan Kosong !'
        });
        ...
        Swal.fire({
            icon: 'error',
            title: 'Gagal...',
            text: 'Form Foto Dokumentasi Pelaporan Kosong !'
        });
    } else if (document.getElementById("foto").files.length > 5) {
        document.getElementById('foto').classList.add("is-invalid");
        document.getElementById("foto").focus();
        Swal.fire({
            icon: 'error',
            title: 'Gagal...',
            text: 'Form Foto Dokumentasi Pelaporan Maksimal 5 File !'
        });
    } else if (validasi_foto() == false) {
        document.getElementById('foto').classList.add("is-invalid");
        document.getElementById("foto").focus();
        Swal.fire({
            icon: 'error',
            title: 'Gagal...',
            text: 'Format Foto Dokumentasi Pelaporan harus .jpg atau .jpeg dan kurang dari 2 MB !'
        });
        ...
        formData.append('jumlah_dokumentasi', document.getElementById('foto').files.length);

        var totalFilesLen = document.getElementById('foto').files.length;
        for (var i = 0; i < totalFilesLen; i++) {
            formData.append("dokumentasi" + "_" + String(i), document.getElementById('foto').files[i]);
        }

        $.ajax({
            url: "<?= base_url() ?>/<?= session()->get('role') ?>/tiket/set_jaringan",
            type: "POST",
            data: formData,
            contentType: false,
            processData: false,
...
function validasi_foto() {
    var fi = document.getElementById('foto');
    var hasil = true;
    if (fi.files.length > 0) {
        var count = 0;
        var total = fi.files.length;
